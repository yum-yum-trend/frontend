...
function trendShow(st, state) {
    $.ajax({
        type: 'GET',
        url: `${WEB_SERVER_DOMAIN}/trend`,
        success: function (response) {
            for (let i = 0; i < response.length; i++) {
                if (response[i].location == "경기" && state == "gygg") {
                    st.animate({fill: '#' + response[i].color}, 200);
                } else if (response[i].location == "서울" && state == "seoul") {
                    st.animate({fill: '#' + response[i].color}, 200);
                } else if (response[i].location == "인천" && state == "incheon") {
                    st.animate({fill: '#' + response[i].color}, 200);
                } else if (response[i].location == "강원" && state == "gangwon") {
                    st.animate({fill: '#' + response[i].color}, 200);
                } else if (response[i].location == "충북" && state == "chungbuk") {
                    st.animate({fill: '#' + response[i].color}, 200);
                } else if (response[i].location == "대전" && state == "daejeon") {
                    st.animate({fill: '#' + response[i].color}, 200);
                } else if (response[i].location == "세종" && state == "sejong") {
                    // 자료 없음
                    st.animate({fill: '#' + response[i].color}, 200);
                } else if (response[i].location == "충남" && state == "chungnam") {
                    st.animate({fill: '#' + response[i].color}, 200);
                } else if (response[i].location == "전북" && state == "jeonbuk") {
                    // 자료 없음
                    st.animate({fill: '#' + response[i].color}, 200);
                } else if (response[i].location == "광주" && state == "gwangju") {
                    st.animate({fill: '#' + response[i].color}, 200);
                } else if (response[i].location == "전남" && state == "jeonnam") {
                    st.animate({fill: '#' + response[i].color}, 200);
                } else if (response[i].location == "경북" && state == "gyeongbuk") {
                    st.animate({fill: '#' + response[i].color}, 200);
                } else if (response[i].location == "대구" && state == "daegu") {
                    st.animate({fill: '#' + response[i].color}, 200);
                } else if (response[i].location == "경남" && state == "gyeongnam") {
                    // 자료 없음
                    st.animate({fill: '#' + response[i].color}, 200);
                } else if (response[i].location == "울산" && state == "ulsan") {
                    // 자료 없음
                    st.animate({fill: '#' + response[i].color}, 200);
                } else if (response[i].location == "부산" && state == "busan") {
                    st.animate({fill: '#' + response[i].color}, 200);
                } else if (response[i].location == "제주특별자치도" && state == "jeju") {
                    st.animate({fill: '#' + response[i].color}, 200);
                }
            }
        },
...
function locationChart(locationName) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            let labelInfo = [];
            for (let i = 0; i < response.length; i++) {
                if (response[i].numberOfOrderByCategoryName > 5 && (!locationName)) {
                    dataInfo.push(response[i].numberOfOrderByCategoryName)
                    labelInfo.push(response[i].categoryName);
                } else if (response[i].numberOfOrderByCategoryName > 0 && locationName) {
                    dataInfo.push(response[i].numberOfOrderByCategoryName)
                    labelInfo.push(response[i].categoryName);
                }
            }
